...
{
    public class GetCrawlJobDataBehaviour : KafkaProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(KafkaConstants.KeyName.ApiKey, nameof(KafkaCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task KafkaCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Kafka.Core.KafkaCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
